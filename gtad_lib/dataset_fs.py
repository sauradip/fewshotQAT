...
class VideoEpisodicDataSet(data.Dataset):
    def __init__(self, opt, subset="val", mode="test"):
        ...
        self.subset = subset
        self.mode = mode
        self.is_multi = opt["multi_instance"]
        ...
        self.video_anno_path = "/home/phd/Desktop/sauradip_research/TAL/gtad/data/activitynet_annotations/anet_anno_action.json"
         # to do ---> same as self.data_list in RePri
        # self._get_match_map()
        self.istrimmed = opt["is_trimmed"]
    ...
    def _getAnntsCwise(self,annts):
        ...
        if self.shot == 5 and self.is_multi :
            time_thres = 0.2
        elif self.shot == 1 and self.is_multi:
            time_thres = 0.1
        elif self.shot == 1 and not self.is_multi:
            time_thres = 0
        else:
            time_thres = 0
        ...
        for i, v in enumerate(self.video_list):
            ...
            for a in annts[v]['annotations']:
                ...
                if flag == 1:
                    if cn in base_class and (corr_end - corr_start) > 0.15: 
                    # if cn in test_class and (corr_end - corr_start) > 0.45:
                    # if cn in test_class:
                        cnt+=1
                    ...
                    if cn in test_class and (corr_end - corr_start) > time_thres: 
                    # if cn in test_class and (corr_end - corr_start) > 0.45:
                    # if cn in test_class:
                        cnt+=1
                        if cn not in annts_cwise:
                        ...
                        # annts_cwise[cn].append([i, a['segment'][0], a['segment'][1]])
                        annts_cwise[cn].append(v) ##just index
        # print("No of Videos less than 2000 frames is ", cnt)
        return annts_cwise
    ...
    def _getDatasetDictEpisode(self):
        ...
        for i in range(len(anno_df)):
            ...
            video_frame = video_info['feature_frame']
            video_subset = anno_df.subset.values[i]
            ## to do : 1) check unique label class 2) check FG class in test split
            ## 3) check if < 768 frames or not 
            if len(video_info["annotations"]) > 1 and  self.is_multi:
                #### common multi-instance ####
                labels1 = str(video_info["annotations"][0]["label"])     
                if flag == 1:
                    # print("cnames", self.cnames) 
                    if labels1 in self.cnames and self.subset in video_subset :
                        self.video_dict[video_name] = video_info
                elif flag == 2:
                    if labels1 in self.cnames and self.subset in video_subset :
                        self.video_dict[video_name] = video_info
            elif len(video_info["annotations"]) == 1: 
                #### common instance ####
                labels1 = str(video_info["annotations"][0]["label"])     
                if flag == 1:
                    # print("cnames", self.cnames)
    ...
    def load_episodic_data(self,index):
        ...
        # print(self.subset)
        if self.subset == "training":
            class_chosen = random.choice(base_class) ## query label
        elif self.subset == "validation":
        ...
        for k in range(shot):
            ...
            if self.istrimmed:
                support_label = torch.ones(100, 1)
            else:
